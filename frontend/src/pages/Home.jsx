...
export default function Home() {
  ...
  return <Layout className="main-layout">
    ...
    <main className="content-area">
      <section className="page-heading"><div><Typography.Title level={2}>{greeting}，{user?.nickname || user?.username}</Typography.Title><Typography.Paragraph>{user?.role === 'teacher' ? '以下是班级近期词汇学习与考试情况。' : '以下是你的词汇学习进度与近期考试安排。'}</Typography.Paragraph></div><Space><Button type="primary" onClick={() => user?.role === 'teacher' ? showExamForm() : document.getElementById('exam-list')?.scrollIntoView({ behavior: 'smooth' })}>{user?.role === 'teacher' ? '发布考试' : '查看考试'}</Button><Button onClick={() => navigate('/history')}>查看成绩</Button></Space></section>

      <Row gutter={[16, 16]} className="metric-grid">
        <Col xs={12} lg={6}><Card className="metric-card metric-navy"><Statistic title={user?.role === 'teacher' ? '全班交卷次数' : '我的考试次数'} value={overview.exam_count || 0} /></Card></Col>
        <Col xs={12} lg={6}><Card className="metric-card metric-blue"><Statistic title="平均成绩" value={overview.average_score || 0} suffix="分" /></Card></Col>
        <Col xs={12} lg={6}><Card className="metric-card metric-green"><Statistic title="历史最高" value={overview.highest_score || 0} suffix="分" /></Card></Col>
        <Col xs={12} lg={6}><Card className="metric-card metric-gold"><Statistic title="进行中考试" value={runningCount} /></Card></Col>
      </Row>

      <Row gutter={[20, 20]} className="dashboard-row">
        <Col xs={24} lg={15}><Card title="近期成绩趋势" extra={<span className="card-note">最近 {trend.length} 次记录</span>} className="data-card"><div className="trend-chart">{trend.length ? trend.map((item) => <div className="trend-item" key={item.id} title={`${item.exam} · ${item.score}分`}><span className="trend-value">{item.score}</span><div className="trend-track"><i style={{ height: `${Math.max(item.score, 8)}%` }} /></div><small>{new Date(item.date).getMonth() + 1}/{new Date(item.date).getDate()}</small></div>) : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="完成考试后生成趋势" />}</div></Card></Col>
        <Col xs={24} lg={9}><Card title="学习成绩排行" extra={<span className="card-note">按平均分排序</span>} className="data-card leaderboard"><List dataSource={leaderboard.slice(0, 5)} locale={{ emptyText: '暂无排行数据' }} renderItem={(item) => <List.Item><List.Item.Meta avatar={<span className={`rank rank-${item.rank}`}>{item.rank}</span>} title={item.student} description={`${item.exam_count} 次考试，最高 ${item.highest_score} 分`} /><strong>{item.average_score}</strong></List.Item>} /></Card></Col>
      </Row>

      <Card title={user?.role === 'teacher' ? '班级薄弱词汇' : '我的薄弱词汇'} extra={<span className="card-note">统计周期：近 30 天</span>} className="section-card data-card"><div className="word-cloud rich-cloud">{words.length ? words.map((word) => <button type="button" key={word.word} onClick={() => setSelectedWord(word)} title={`${word.meaning} · 错误 ${word.weight} 次`} style={{ fontSize: 15 + Math.min(word.weight, 8) * 4 }}>{word.word}</button>) : <Typography.Text type="secondary">完成考试并产生错题后，这里会显示需要重点复习的词汇。</Typography.Text>}</div></Card>

      <Card id="exam-list" title="考试安排" className="section-card data-card" extra={user?.role === 'teacher' && <Button type="primary" onClick={() => showExamForm()}>发布考试</Button>}><Segmented value={filter} onChange={setFilter} options={['全部', '进行中', '未开始', '已结束']} className="exam-filter" />
